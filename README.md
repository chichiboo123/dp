...
- **파일 불러오기** — Google Drive에서 PDF·JPG·PNG 파일을 선택해 로드 (이미지는 1쪽짜리 학습지로 처리)
- **오프라인 모드(로컬 파일 열기)** — 구글 로그인 없이 [로컬에서 열기]로 내 기기의 PDF·JPG·PNG 파일을 바로 열어 필기·입력창·내보내기까지 동일하게 사용 가능. 다만 클라우드 사본이 없어 URL 기반 [공유] 링크는 만들 수 없음(학생에게는 JPG/PDF로 저장해 전달하거나 구글 드라이브에 올린 뒤 공유)
- **자유 필기** — 곡선 보정(스무딩)과 필압(스타일러스)·속도 기반 굵기 변화로 부드러운 필감의 펜, 반투명 형광펜, 크기 미리보기가 표시되는 지우개. 펜 사용 중 손바닥 터치는 무시(팜 리젝션)
- **텍스트 입력창** — 교사가 드래그로 답 칸을 만들고, 학생은 그 칸에 글자를 입력. 한글 구글폰트·글자 크기·색상·굵기·정렬을 Canva 스타일 편집 막대로 지정. 옵션을 켜서 공유하면 학생도 직접 텍스트 상자를 만들 수 있음
- **체크박스** — 교사가 클릭으로 선택 칸을 배치하고, 학생은 눌러서 체크/해제
- **멀티 페이지** — 페이지별 독립 필기 레이어 관리
- **실행 취소/다시 실행** — 페이지당 최대 60단계 히스토리
- **확대/축소** — 50~400% 줌, 너비 맞춤, 모바일 핀치 제스처 지원. PC에서는 문서 위에 마우스를 올리고 휠을 굴리면 확대/축소(문서 밖에서는 일반 스크롤)
...
| 도구 | 아이콘 | 설명 |
|------|--------|------|
| 펜 / 형광펜 / 지우개 | ✏️ / 🖍️ / 🧽 | 자유 필기·강조·지우기 (색상 프리셋과 굵기 조절, 필압 지원) |
...
1. 구글 로그인 후 Google Drive에서 PDF·JPG·PNG 파일 선택 — 또는 구글 로그인이 막혀 있을 때는 **[로컬에서 열기]**로 내 기기의 파일을 바로 불러와도 됨 (단, 이 경우 URL 공유는 불가하고 JPG/PDF로 내보내 전달해야 함)
...
7. 상단의 **공유** 버튼을 누르면 공유 대화상자가 열림. 여기서 **학생도 텍스트 상자 만들기 허용** 옵션을 선택할 수 있음 (기본은 꺼짐 — 교사가 만든 칸만 채우기). **[링크 복사]**를 누르면 만든 칸과 설정이 함께 담긴 링크가 복사됨 → 학생에게 배포

### 학생 (입력 및 제출)

1. 교사가 공유한 링크를 열면 로그인 없이 PDF와 입력 칸이 바로 표시됨
2. 글상자를 눌러 답을 쓰고(글꼴·크기·색상 변경 가능), 체크박스를 눌러 체크. 교사가 허용한 경우 **글상자** 도구로 직접 새 칸을 만들 수도 있음
