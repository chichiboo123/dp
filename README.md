...
| 체크박스 | ☑️ | *교사 전용* — 클릭한 위치에 체크 칸 배치 (같은 칸을 다시 클릭하면 제거) |
| 이동 | ✋ | 확대 상태에서 화면을 끌어 이동 |
| 초기화 | ♻️ | 현재 페이지의 필기·글상자·체크박스를 한 번에 제거 |

> - 별도의 "선택" 버튼은 없습니다. **아무 도구도 켜지 않은 상태가 곧 입력/선택 모드**입니다. 켜져 있는 도구를 한 번 더 누르면 입력 모드로 돌아갑니다.
> - 글상자/체크박스 **생성·이동·삭제는 교사(원본 화면)**만, **입력·체크는 학생(공유 링크)**도 할 수 있습니다.
...
4. 빈 곳을 **드래그하면 여러 칸을 한 번에 선택**해 함께 이동하거나 한꺼번에 삭제할 수 있음
5. 필요하면 펜으로 예시 필기 추가
6. (선택) **제출 링크 설정**에서 Padlet·Canva 등 제출처 URL 입력
7. 상단의 **공유** 버튼으로 링크 복사 → 학생에게 배포 (만든 칸이 링크에 함께 담김)

### 학생 (입력 및 제출)

1. 교사가 공유한 링크를 열면 로그인 없이 PDF와 입력 칸이 바로 표시됨
2. 글상자를 눌러 답을 쓰고(글꼴·크기·색상 변경 가능), 체크박스를 눌러 체크
3. **펜** 도구로 자유롭게 필기 (손가락 두 개로 확대/축소). 다시 입력하려면 켜진 펜을 한 번 더 눌러 끄기
