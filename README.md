...
5. 필요하면 펜으로 예시 필기 추가
6. (선택) **제출 링크 설정**에서 Padlet·Canva 등 제출처 URL 입력. 같은 화면의 **학생도 텍스트 상자 만들기 허용** 옵션을 켜면 학생이 공유 링크에서 직접 글상자를 추가할 수 있음 (기본은 꺼짐 — 교사가 만든 칸만 채우기)
7. 상단의 **공유** 버튼으로 링크 복사 → 학생에게 배포 (만든 칸과 설정이 링크에 함께 담김)
